...
const AllFunoonPage = () => {
  ...
  if (!funoon) return null;

  const totalPages = Math.ceil(funoon.length / ITEMS_PER_PAGE);
  const paginatedItems = funoon.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE);
          ...
          >
            <div className="group cursor-pointer active:scale-95 transition-transform duration-150">
              <div className="relative overflow-hidden rounded-xl">
                <div className="aspect-square w-full bg-gradient-to-br from-accent/20 to-primary/20" />
                <div className="absolute inset-0 bg-gradient-to-t from-primary/60 via-transparent to-transparent" />
                <div className="absolute bottom-3 start-3 end-3">
                  <h3 className="font-fustat text-sm font-bold text-white">{fn.name}</h3>
                </div>
              </div>
            </div>
          </motion.div>
